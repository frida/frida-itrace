...
#define MAX_WRITES_PER_BLOCK 4096
           ...
           gpointer user_data)
{
  ...
  ITraceBlockWrite writes[MAX_WRITES_PER_BLOCK];
  guint num_writes = 0;
  ...
  while (gum_stalker_iterator_next (iterator, &insn))
  {
    ...
    if (is_first_in_block)
    {
      ...
      writes[num_writes].block_offset = insn->address - block_address;
      writes[num_writes].reg_index = 33;
      num_writes++;
    }
    ...
    for (uint8_t i = 0; i != num_regs_written; i++)
    {
      ...
      writes[num_writes].block_offset = block_offset;
      writes[num_writes].reg_index = cpu_reg_index;
      num_writes++;
  ...
  gsize writes_size = num_writes * sizeof (ITraceBlockWrite);
  ...
  /* Writes array */
  memcpy (p, writes, writes_size); p += writes_size;
                    ...
                    gpointer user_data)
{
  ...
  {
    gchar name[4];
    g_snprintf (name, sizeof (name), "x%u", i);
  ...
  {
    gchar name[4];
    g_snprintf (name, sizeof (name), "v%u", i);
